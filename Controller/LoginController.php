<?php 
    ...
    */
    class loginController {
        ...
        public function login($arr_data) {
        ...
        public function store($arr_data) {
            ...
            $email              = $arr_data['email'];
            $password           = $arr_data['password'];
            $password_confirm   = $arr_data['password_confirm'];

            if ($name != null && $email != null && $password == $password_confirm) {
                $arr_data = array(
                    'name'          => $name,
                    'email'         => $email,
                ...
                setcookie('msg', '<a href="#" class="logo" style="margin-left: 5%; color: black;"><font>Chào mừng ' . "<b>" . $user['name'] . "</b>" . ' đến với Framgia - Education. Chúc bạn một ngày tốt lành!</font></a>', time() + 15);

                header('Location: /index.php');
            } else {
                setcookie('error', '<center><font style="color: red;"><b><br>Password không khớp!<br>Bạn vui lòng nhập lại.</b></font></center>', time() + 5);

                header('Location: /index.php?module=login&act=create');
            }
        }
